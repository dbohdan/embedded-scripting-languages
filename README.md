...
| [Dhall](https://github.com/dhall-lang/dhall-lang/wiki/How-to-integrate-Dhall) | Haskell | Haskell's GC | A functional configuration language.  Not Turing-complete.  Untrusted code: can't access the file system, can fetch Dhall libraries over HTTP(S) from static URLs (but libraries aren't allowed to access your data), can use up memory and CPU time for a DoS attack. |
...
| [Jsonnet](https://jsonnet.org/) | C++ | Tracing | Apache License 2.0 | A functional configuration language that extends JSON.  Untrusted code can't access the system, can use up memory and CPU time for a DoS attack.  Has [bindings](https://jsonnet.org/ref/bindings.html) for C, C++, Go, Python, and other languages, as well as a separate native Go implementation. |
...
| [Starlark](https://github.com/google/starlark-go/) (Go) | Go | Go's GC | 3-clause BSD | A small dialect of Python for configuration.  Untrusted code can't access the system, can use up memory and CPU to perform a DoS attack.  [Spec](https://github.com/bazelbuild/starlark). |
